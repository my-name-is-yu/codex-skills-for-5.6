...
Write each entry the way you'd tell a teammate what you did. Use plain words and concrete actions; remove AI-shaped filler and abstract jargon. A reviewer should understand each row without decoding it.
...
- Prefer evidence produced by reusable scripts over hand-made one-offs so a reviewer can rerun it.
...
- Every row maps to a real action. If an existing row is invented or aspirational, append a correction that supersedes it; do not rewrite history.
- Each row's evidence resolves and shows what the row claims.
- A fork, pivot, or abandoned approach that shaped the work but isn't logged is a gap. Add it.
- Flag padding and stop adding it. Existing padding remains part of the append-only record.

Correct the log, not the story. If the work diverged from what a row claims, append a row that supersedes the wrong claim and points to the evidence.

## Independent review of the trail

Before handing back, request an independent review through an installed profile with a different observable model family when available. Otherwise use a generic independent agent and label its model inherited or unverified. If independent agents are unavailable, do not claim independent or cross-model review; report that limitation. The reviewer reads the audit trail and supported task history or digest, then flags what the user should inspect.
...
Every reply for a run that produced a trail ends with an "Attention" section. Lead with the reviewer's observed model status on its own line (`reviewed by <model, inherited, or unverified>`), then list each flag pointing to specific rows or moments. "No flags" is a valid value; the model status is not. The self-audit asks if the log told the truth; this asks what the user should still scrutinize even when it did.
