...
1. Read the change. The diff, the symbols it adds, changes, and deletes, and what it now does differently, including the part the diff doesn't spell out. Use the `why` skill's code-anchor step when available; otherwise inspect local git and accessible PR evidence directly and label any gaps.
...
6. For a big or wide change, give the same evidence bundle and risk question to two or three independent read-only reviewers, then verify and merge their findings in the parent. Use inherited agents unless validated profiles are installed. Report reviewer or model diversity only when the runtime can prove it; otherwise label the lanes independent but same-model or unverified.
...
Write in plain, concrete language, cite real code, and strip anything private before it goes anywhere public.
